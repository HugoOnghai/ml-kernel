...
// Implement the OPTIMIZED linear layer function
// helpful source: https://www.codeproject.com/articles/Crunching-Numbers-with-AVX-and-AVX#comments-section
float *linear_AVX(float *fcInput, float **weights, float *biases, int fcInputSize, int fcOutputSize) {
    float *output = (float *)malloc(fcOutputSize * sizeof(float));

    // (1xfcInputSize)*(fcInputSizexfcOutputSize) + (1xfcOutputSize) = (1xfcOutputSize)

    for (int i = 0; i < fcOutputSize; i++) {
        __m256 acc = _mm256_setzero_ps();
        float *w = weights[i]; // grab one of fcOutputSize rows, which have fcInputSize elements each
        int j = 0;

        // for loop through 8 elements per vector at a time
        for (j = 0; j + 8 - 1 < fcInputSize; j += 8) {
            __m256 wvec = _mm256_loadu_ps(&w[j]);
            __m256 xvec = _mm256_loadu_ps(&fcInput[j]);
            acc = _mm256_fmadd_ps(wvec, xvec, acc); // acc += wvec * xvec
        }
