...
#include <string.h>
#include <time.h>
#include <immintrin.h> // including this to access the AVX functions
...
// My new functions for optimized performance
float *linear_AVX(float *fcInput, float **weights, float *biases, int fcInputSize, int fcOutputSize);

static inline float hsum256_ps(__m256 v) {
    // Reduce 8 floats in v to a single float without storing to memory
    __m128 lo = _mm256_castps256_ps128(v);               // low 128
    __m128 hi = _mm256_extractf128_ps(v, 1);            // high 128
    lo = _mm_add_ps(lo, hi);                            // pairwise add -> 4 floats
    lo = _mm_hadd_ps(lo, lo);                           // hadd -> 2 floats repeated
    lo = _mm_hadd_ps(lo, lo);                           // hadd -> scalar in lowest lane
    return _mm_cvtss_f32(lo);
}

float *linear_AVX(float *fcInput, float **weights, float *biases, int fcInputSize, int fcOutputSize) {
    // allocate output (optionally align for faster stores/loads elsewhere)
    float *output = (float *)malloc(fcOutputSize * sizeof(float));
    if (!output) return NULL;

    const int simd = 8;    // floats per AVX2 vector
    int j, i;

    // outer loop: each output row is independent -> can be parallelized
    // consider: #pragma omp parallel for if you compile with -fopenmp and include <omp.h>
    for (i = 0; i < fcOutputSize; ++i) {
        float *w = weights[i];
        __m256 acc0 = _mm256_setzero_ps();
        __m256 acc1 = _mm256_setzero_ps();

        // process 16 elements per iteration (two 8-wide vectors)
        for (j = 0; j + 2*simd - 1 < fcInputSize; j += 2*simd) {
            __m256 wvec0 = _mm256_loadu_ps(&w[j]);
            __m256 xvec0 = _mm256_loadu_ps(&fcInput[j]);
            acc0 = _mm256_fmadd_ps(wvec0, xvec0, acc0);

            __m256 wvec1 = _mm256_loadu_ps(&w[j + simd]);
            __m256 xvec1 = _mm256_loadu_ps(&fcInput[j + simd]);
            acc1 = _mm256_fmadd_ps(wvec1, xvec1, acc1);
        }

        // process remaining full simd blocks
        for (; j + simd - 1 < fcInputSize; j += simd) {
            __m256 wvec = _mm256_loadu_ps(&w[j]);
            __m256 xvec = _mm256_loadu_ps(&fcInput[j]);
            acc0 = _mm256_fmadd_ps(wvec, xvec, acc0);
        }
        ...
        output[i] = (biases ? biases[i] : 0.0f) + sum;
    }
...
float **block(float **x, int seqLength, int embeddingSize, BlockWeights weights) {
    ...
    for (int i = 0; i < seqLength; i++) {
        Q[i] = linear_AVX(normalized_x[i], q_mlp.weights, q_mlp.biases, q_mlp.fcInputSize, q_mlp.fcOutputSize);
        K[i] = linear_AVX(normalized_x[i], k_mlp.weights, k_mlp.biases, k_mlp.fcInputSize, k_mlp.fcOutputSize);
        V[i] = linear_AVX(normalized_x[i], v_mlp.weights, v_mlp.biases, v_mlp.fcInputSize, v_mlp.fcOutputSize);
    }
    ...
    for (int i = 0; i < seqLength; i++) {
        ...
        float *hidden = linear_AVX(normalized_x_added[i], first_block_MLP.weights, first_block_MLP.biases, first_block_MLP.fcInputSize, first_block_MLP.fcOutputSize);
        float *activated = gelu(hidden, first_block_MLP.fcOutputSize);
        m[i] = linear_AVX(activated, second_block_MLP.weights, second_block_MLP.biases, second_block_MLP.fcInputSize, second_block_MLP.fcOutputSize);
    }
...
float *model(int *tokens, int seqLength, GPT2Weights weights) {
    ...
    float *logits = linear_AVX(h[seqLength - 1], logits_mlp.weights, logits_mlp.biases, logits_mlp.fcInputSize, logits_mlp.fcOutputSize);
...
// srun -p ece5755 -n 1 --mem=1g --pty /bin/bash
// python /classes/ece5755/pmu-tools/toplev.py --core S0-C0 -l2 -v -no-desc --force-cpu spr ./gpt2
// gcc gpt2_opt.c -o gpt2_opt -lm -mavx2 -mfma -coverage